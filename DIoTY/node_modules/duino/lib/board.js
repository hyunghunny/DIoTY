  ...
  this.detect(function (err, serial) {
    if (err) {
      if(self.listeners('error').length) {
        ...
        self.log('info', 'No device found: ' + err);
...
Board.prototype.detect = function (callback) {
  ...
  child.exec('ls /dev | grep -E "' + self.device + '"', function (err, stdout, stderr) {
      ...
      if (stderr) {
        // fallback to the windows OS system
        temp = new serial.SerialPort( self.device, {
            baudrate: self.baudrate,
            parser: serial.parsers.readline('\n')
        }, false);

        temp.open(function (err) {
          if (!err) {
              found = temp;

              self.log('info', 'found board at ' + self.device);
              callback(err, found);
          } else {
